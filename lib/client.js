window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		const THINKING_TOKEN_BUDGET_FIELDS = [
		...
		const TOOL_UPDATES = ["in-history", "addition-only"];
		...
		*/
		...
		function KeyValueList(props) {
			...
			}, e$14(_deepseek_ai_dsh_client_ui_primitives.IconCloseOutlineRegular, { size: 16 })))), e$14("button", {
				type: "button",
				className: "dsh-ma-button",
				disabled: props.disabled === true || levels && entries.length === THINKING_LEVELS.length,
				style: { marginTop: "8px" },
				onClick: add
			}, e$14(_deepseek_ai_dsh_client_ui_primitives.IconPlusOutlineRegular, { size: 14 }), tr(levels ? "controls.keyValue.efforts.add" : "controls.keyValue.headers.add")));
		}
		//#endregion
		...
		function CompatEditor(props) {
			...
			else if (props.api === "openai-responses") gridContent = e$13("div", { className: "dsh-ma-grid" }, boolSelect("supportsDeveloperRole", "controls.compat.supportsDeveloperRole"), boolSelect("supportsStrictMode", "controls.compat.supportsStrictMode"), boolSelect("supportsMaxOutputTokens", "controls.compat.supportsMaxOutputTokens"), boolSelect("supportsLongCacheRetention", "controls.compat.supportsLongCacheRetention"));
			...
			})), e$13(Field, { labelKey: "controls.compat.vllmPriority" }, e$13(TextInput, {
				type: "number",
				step: 1,
				value: compat.vllmPriority,
				placeholderKey: "controls.compat.vllmPriorityPlaceholder",
				disabled: props.disabled,
				onChange: (value) => set("vllmPriority", typeof value === "number" && Number.isFinite(value) ? Math.trunc(value) : void 0)
			})));
		...
		function RetryPolicy(props) {
			const stored = isObject(props.value) ? props.value : {};
			const mode = stored.mode === "always" ? "always" : "normal";
			const maxRetries = typeof stored.maxRetries === "number" ? stored.maxRetries : void 0;
			const retryableCodes = Array.isArray(stored.retryableCodes) ? stored.retryableCodes : void 0;
			const backoff = isObject(stored.backoff) ? stored.backoff : {};
			/** Emit one well-formed policy, defaulting the bounded-only fields. */
			const emit = (next) => props.onChange?.(next.mode === "always" ? {
				mode: "always",
			...
			const setBackoff = (field, value) => emit({
				mode,
				backoff: {
					...backoff,
					[field]: value
			...
			return e$11("div", { className: "dsh-ma-grid dsh-ma-wide" }, e$11(Field, { labelKey: "controls.retry.modeLabel" }, e$11(Select, {
				...
				onChange: (next) => emit({ mode: next === "always" ? "always" : "normal" })
			})), mode === "normal" ? e$11(Field, { labelKey: "controls.retry.maxRetries" }, e$11(TextInput, {
				type: "number",
				min: 0,
				step: 1,
				value: maxRetries,
				...
				onChange: (value) => emit({
					mode: "normal",
					maxRetries: typeof value === "number" ? value : void 0
				})
			})) : null, mode === "normal" ? e$11(Field, {
				labelKey: "controls.retry.retryableCodes",
				wide: true
			}, e$11(TextInput, {
				value: Array.isArray(retryableCodes) ? retryableCodes.join(", ") : "",
				disabled: props.disabled,
				emptyAsUndefined: false,
				onChange: (value) => emit({
					mode: "normal",
					retryableCodes: String(value).split(/[,\s]+/).map((item) => item.trim()).filter(Boolean)
				})
...
`;
		...
		function CredentialField({ api, keyRef, revision, value, onChange, disabled }) {
			...
			react.useEffect(() => {
				...
				api.credentials.describe([keyRef]).then((response) => {
					if (active && response.result.ok) setState(response.result.value?.[keyRef]);
				}, () => void 0);
				return () => {
					active = false;
				};
		...
		*/
		function disclosedModalities(candidate) {
			return Array.isArray(candidate.inputModalities) ? candidate.inputModalities.filter((item) => MODALITIES.includes(item)) : [];
		}
		/**
		...
		*/
		function discoveredModelProfile(candidate) {
		...
		*/
		...
		function ModelDiscoveryDialog({ probe, existing, onApply, onClose }) {
			...
			return e$9(_deepseek_ai_dsh_client_ui_primitives.Modal, {
				...
				const modalities = disclosedModalities(candidate);
				const facts = [
					typeof candidate.contextWindow === "number" ? tr("models.discovery.contextWindow", { value: formatCapacity(candidate.contextWindow) }) : null,
					typeof candidate.maxTokens === "number" ? tr("models.discovery.maxTokens", { value: formatCapacity(candidate.maxTokens) }) : null,
					modalities.length > 0 ? tr("models.discovery.inputModalities", { value: modalities.map((item) => tr(`controls.modality.${item}`)).join(", ") }) : null
				].filter(Boolean);
				return e$9("label", {
					key: candidate.id,
					className: "dsh-ma-check",
				...
				}), e$9("span", null, candidate.name ? `${candidate.name} (${candidate.id})` : candidate.id), facts.length > 0 ? e$9("span", { className: "dsh-ma-route" }, facts.join(" · ")) : null, exists ? e$9("span", { className: "dsh-ma-route" }, tr("models.discovery.existing")) : null);
			}))) : null);
		}
		//#endregion
		...
		function ModelList({ value, onChange, disabled, override, probe, api }) {
			...
			}, e$7(_deepseek_ai_dsh_client_ui_primitives.IconCopyOutlineRegular, { size: 16 })), e$7("button", {
				type: "button",
				className: "dsh-ma-button dsh-ma-icon",
				disabled,
			...
			}, e$7(_deepseek_ai_dsh_client_ui_primitives.IconCloseOutlineRegular, { size: 16 })))), e$7(ModelForm, {
				model,
				includeId: !override,
				disabled,
				api,
			...
			}, e$7(_deepseek_ai_dsh_client_ui_primitives.IconPlusOutlineRegular, { size: 14 }), tr(override ? "models.action.addOverride" : "models.action.add")), !override && probe ? e$7("button", {
				type: "button",
				className: "dsh-ma-button",
		...
		function OfficialModelList({ value, onChange, disabled }) {
			...
			return e$6("div", { className: "dsh-ma-wide" }, models.map((model, index) => {
				...
				return e$6("div", {
				...
				}, e$6(_deepseek_ai_dsh_client_ui_primitives.IconCopyOutlineRegular, { size: 16 })), e$6("button", {
					type: "button",
					className: "dsh-ma-button dsh-ma-icon",
					disabled,
					title: tr("models.action.remove"),
				...
				}, e$6(_deepseek_ai_dsh_client_ui_primitives.IconCloseOutlineRegular, { size: 16 })))), e$6("div", { className: "dsh-ma-grid" }, e$6("label", { className: "dsh-ma-field" }, e$6("span", { className: "dsh-ma-field-label" }, tr("models.field.id")), e$6(TextInput, {
					value: model.id,
					disabled,
				...
				})), e$6("label", { className: "dsh-ma-field dsh-ma-wide" }, e$6("span", { className: "dsh-ma-field-label" }, tr("models.field.systemPromptUpdate")), e$6(Select, {
					...
					onChange: (nextValue) => update(index, "systemPromptUpdate", nextValue === "in-history" ? "in-history" : void 0)
				})), e$6("label", { className: "dsh-ma-field dsh-ma-wide" }, e$6("span", { className: "dsh-ma-field-label" }, tr("models.field.toolUpdate")), e$6(Select, {
			...
			}, e$6(_deepseek_ai_dsh_client_ui_primitives.IconPlusOutlineRegular, { size: 14 }), tr("models.action.add")));
		}
		//#endregion
		...
		function validateCompat(value, path, errors) {
			...
			if (owns(value, "thinkingTokenBudgetField") && !THINKING_TOKEN_BUDGET_FIELDS.includes(String(value.thinkingTokenBudgetField))) errors.push(tr("validation.invalid", { field: `${path}.thinkingTokenBudgetField` }));
			if (owns(value, "vllmPriority")) numberError(value.vllmPriority, `${path}.vllmPriority`, errors, -Number.MAX_SAFE_INTEGER, Number.MAX_SAFE_INTEGER, true);
		...
		function validateRetryPolicy(value) {
			const errors = [];
			if (!isObject(value) || value.mode !== "normal" && value.mode !== "always") return [tr("validation.retry")];
			if (value.backoff !== void 0 && !isObject(value.backoff)) return [tr("validation.retry")];
			...
			]) if (owns(backoff, field)) numberError(backoff[field], `field.${field}`, errors, min, max, false);
			if (owns(backoff, "initialDelayMs") && owns(backoff, "maxDelayMs") && Number(backoff.initialDelayMs) > Number(backoff.maxDelayMs)) errors.push(tr("validation.backoffOrder"));
			if (value.mode === "normal") {
				if (owns(value, "maxRetries")) numberError(value.maxRetries, "field.maxRetries", errors, 0, Number.MAX_SAFE_INTEGER, true);
				if (owns(value, "retryableCodes") && (!Array.isArray(value.retryableCodes) || value.retryableCodes.length === 0 || value.retryableCodes.some((code) => typeof code !== "string" || code.trim().length === 0) || new Set(value.retryableCodes).size !== value.retryableCodes.length)) errors.push(tr("validation.retryCodes"));
			}
			return errors;
		}
				...
				if (!Array.isArray(profile.models)) errors.push(tr("validation.models"));
					...
					profile.models.forEach((entry, index) => {
						...
						if (isObject(entry)) {
							...
							if (owns(entry, "systemPromptUpdate") && entry.systemPromptUpdate !== "in-history") errors.push(tr("validation.invalid", { field: `${path}.systemPromptUpdate` }));
							if (owns(entry, "toolUpdate") && !TOOL_UPDATES.includes(entry.toolUpdate)) errors.push(tr("validation.invalid", { field: `${path}.toolUpdate` }));
							...
							const modalities = owns(entry, "inputModalities") ? entry.inputModalities : void 0;
							if (owns(entry, "input")) errors.push(tr("validation.invalid", { field: `${path}.input` }));
							...
							if (!(Array.isArray(modalities) && modalities.includes("image"))) {
								if (owns(entry, "imagePixelBudget") || owns(entry, "imageMaxBytes")) errors.push(tr("validation.textOnlyImageLimits"));
							} else {
		...
		function AdvancedModelsPage({ api, retryLater, timeout, subscribe }) {
			...
			return e$1("div", { className: "dsh-ma-page" }, e$1("div", { className: "dsh-ma-header" }, e$1("h2", { className: "dsh-ma-title" }, tr("title")), e$1("div", { className: "dsh-ma-toolbar" }, e$1("button", {
				...
				onClick: load
			}, e$1(_deepseek_ai_dsh_client_ui_primitives.IconRefreshOutlineRegular, { size: 16 })), e$1("button", {
				type: "button",
				className: "dsh-ma-button",
				disabled: !state.writable || state.status !== "ready",
				onClick: () => setCreateMode((value) => value === "builtin" ? void 0 : "builtin")
			}, e$1(_deepseek_ai_dsh_client_ui_primitives.IconPlusOutlineRegular, { size: 14 }), tr("addBuiltIn")), e$1("button", {
				type: "button",
				className: "dsh-ma-button",
				disabled: !state.writable || state.status !== "ready",
				onClick: () => setCreateMode((value) => value === "custom" ? void 0 : "custom")
			}, e$1(_deepseek_ai_dsh_client_ui_primitives.IconPlusOutlineRegular, { size: 14 }), tr("addCustom")))), !state.writable && state.status === "ready" ? e$1("div", { className: "dsh-ma-notice" }, tr("readOnly")) : null, state.status === "loading" ? e$1("p", { className: "dsh-ma-status" }, tr("loading")) : null, state.status === "waiting" ? e$1("p", { className: "dsh-ma-status" }, tr("waiting")) : null, state.status === "error" ? e$1("p", {
				className: "dsh-ma-status dsh-ma-error",
				role: "alert"
